...
A linear response to a Gaussian excitation field will result in a Gaussian field, and is a [Gaussian Processes](https://en.wikipedia.org/wiki/Gaussian_process). The information Hamiltonian for this processes is denoted $$\mathcal{H}_{free}$$. The **posterior mean** of this free IFT is given by 
